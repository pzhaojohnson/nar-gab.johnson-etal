...
class Hit:
    ...
    @property
    def num_hsps(self):
        return len(self.hsps_sorted_by_query_from)
...
def matches_full_length_gRNA(search):
    ...
    return len(hsps) == 1 \
        and HspRange(1, 2983).matches(hsps[0])


def matches_sgRNA(search):
    hsps = search.hit.hsps_sorted_by_query_from
    return len(hsps) == 1 \
        and HspRange(2068, 2983).matches(hsps[0])
...
def matches_DRNA(search):
    ...
    return len(hsps) == 2 \
        ...
        and are_within(2983, hsps[1].hit_to, 10) \
...
for search in searches_with_a_hit:
    ...
    assert search.hit.is_to_CY2()
    #assert search.hit.is_to_rubisco_large()
print('All hits are to CY2.')
...
single_segment_plus_searches = list(filter(lambda search : search.hit.num_hsps == 1, plus_searches))
print(f'{len(single_segment_plus_searches)=}')
print()

multi_segment_plus_searches = list(filter(lambda search : search.hit.num_hsps > 1, plus_searches))
print(f'{len(multi_segment_plus_searches)=}')
print()
...
multi_segment_plus_searches.sort(key=functools.cmp_to_key(cmp_num_unique_covered_pos), reverse=True)

y = 1

for search in multi_segment_plus_searches:
    #color = 'black'
    color = plotted_color(search)
    alpha = 0.5 if color == 'black' else 1
    hsps = search.hit.hsps_sorted_by_query_from
    for hsp in hsps:
