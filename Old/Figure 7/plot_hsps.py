...
def matches_F671(search):
    ...
    return len(hsps) == 1 \
        ...
        and are_within(search.read_length, 671, 60)
...
#blast_output_file_path = 'blast_to_rubisco_large_output_cy2_nb_14wpi_leaf.json'
#blast_output_file_path = 'blast_to_rubisco_large_output_cy2_hemp_leaf.json'
...
print(f'{100 * len(list(filter(matches_sgRNA, plus_searches))) / len(plus_searches)=}')
print()
...
for search in plus_searches:
    y = -search.num_unique_covered_pos
    color = plotted_color(search)
    alpha = 0.2 if color == 'black' else 1
    hsps = search.hit.hsps_sorted_by_query_from
    for hsp in hsps:
